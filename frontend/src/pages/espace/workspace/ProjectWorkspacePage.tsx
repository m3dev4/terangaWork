  ...
  if (isPropositionsLoading) {
    return (
      <div className="flex h-64 flex-col items-center justify-center gap-3">
        ...
        <p className="text-xs text-[#111118]/50">Chargement de votre espace projet...</p>
      </div>
    );
  }
      ...
      <div className="relative overflow-hidden rounded-[28px] bg-[#111118] text-white p-6 sm:p-7">
        <div className="relative z-10 flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {(!activeProject.mission_status || activeProject.mission_status === 'IN_PROGRESS') && (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-2.5 py-0.5 text-[9.5px] font-semibold text-[#E7B84B]">
                  <span className="h-1.5 w-1.5 rounded-full bg-[#E7B84B] animate-pulse" /> En cours de développement
                </span>
              )}
              {activeProject.mission_status === 'DELIVERED' && (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-2.5 py-0.5 text-[9.5px] font-semibold text-white/80 border border-white/15">
                  <span className="h-1.5 w-1.5 rounded-full bg-white animate-pulse" /> Livrée — en attente de validation
                </span>
              )}
          ...
          {user?.role === 'annonceur' && (
            ...
            >
              <ShieldCheck className="h-4 w-4 text-[#E7B84B]" /> Validation livraison & paiement
            </button>
          )}
        </div>
        ...
        <div className="relative z-10 mt-5 flex flex-wrap items-center gap-4 border-t border-white/10 pt-4">
          <div className="flex items-center gap-2">
            ...
            <div className="text-left">
              <p className="text-[10px] text-white/40 font-medium leading-none">Annonceur</p>
              <p className="text-[11px] font-semibold text-white leading-tight">Client</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="text-left">
              <p className="text-[10px] text-white/40 font-medium leading-none">Freelance</p>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-5">
              ...
              <div className="flex items-center gap-1 rounded-xl bg-[#F3EBDD]/60 p-1 text-[10px] font-semibold text-[#111118]/60">
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg transition-all ${
                    calendarView === 'month' ? 'bg-white text-[#111118] shadow-2xs' : 'hover:text-[#111118]'
                  }`}
                >
                  Mois
                </button>
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg transition-all ${
                    calendarView === 'week' ? 'bg-white text-[#111118] shadow-2xs' : 'hover:text-[#111118]'
                  }`}
                >
                  Semaine
                </button>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className={`text-[10px] font-bold ${cell.isCurrentMonth ? 'text-[#111118]/70' : 'text-[#111118]/25'}`}>
                        {cell.dayNumber}
                      </span>
                    </div>
                    ...
                    <div className="space-y-1">
                      {isStart && (
                        <div className="rounded bg-[#111118] px-1 py-0.5 text-[8px] font-bold text-white truncate" title="Début de la collaboration">
                          Début
                        </div>
                      )}
                      {isEnd && (
                        <div className="rounded bg-[#D95C38] px-1 py-0.5 text-[8px] font-bold text-white truncate" title="Livraison prévue">
                          Livraison
                        </div>
                      )}
                      {dayMeets.map((m) => (
                        ...
                        >
                          <Video className="h-2 w-2 shrink-0 text-[#D95C38]" /> {m.time.substring(0, 5)}
                        </a>
                      ))}
                    </div>
                  </div>
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <div className="rounded-[24px] border border-[#111118]/8 bg-white p-5">
              <div className="mb-3 flex items-center justify-between border-b border-[#111118]/6 pb-2.5">
                <h4 className="font-heading text-xs font-bold text-[#111118] flex items-center gap-1.5">
                  <CheckCircle2 className="h-3.5 w-3.5 text-[#D95C38]" /> Progression des étapes
                </h4>
                <span className="text-[9px] font-semibold text-[#111118]/35">4 phases</span>
              </div>
              ...
              {(() => {
                ...
                const steps = [
                  { label: 'Cadrage & spécifications', done: true, active: false },
                  { label: 'Développement & intégration', done: isCompleted || isDelivered, active: s === 'IN_PROGRESS' },
                  { label: 'Recette & tests utilisateurs', done: isCompleted, active: isDelivered },
                  { label: 'Livraison finale & déploiement', done: isCompleted, active: false },
                ];
                return (
                  <div className="space-y-2.5">
                    {steps.map((m, i) => (
                      <div key={i} className="flex items-center justify-between text-[11px]">
                        <span className="font-medium text-[#111118]/75">{m.label}</span>
                        <span
                          className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${
                            m.done
                              ? 'bg-[#F3EBDD] text-[#111118]/70 border border-[#E7B84B]/40'
                              : m.active
                              ? 'bg-[#E7B84B]/20 text-[#c9922e] border border-[#E7B84B]/40 animate-pulse'
                              : 'bg-[#111118]/5 text-[#111118]/35'
                          }`}
                        >
            ...
            <div className="rounded-[24px] border border-[#111118]/8 bg-white p-5">
              <div className="mb-3 flex items-center justify-between border-b border-[#111118]/6 pb-2.5">
                <h4 className="font-heading text-xs font-bold text-[#111118] flex items-center gap-1.5">
                  <PieIcon className="h-3.5 w-3.5 text-[#D95C38]" /> Répartition des tâches
                </h4>
              </div>
              ...
              {(() => {
                ...
                const ringClass = isCompleted
                  ? 'border-[#E7B84B]'
                  : 'border-[#111118] border-t-[#D95C38] border-r-[#E7B84B]';
                return (
                  <div className="flex items-center gap-4">
                    <div className={`relative h-16 w-16 shrink-0 rounded-full border-4 ${ringClass} flex items-center justify-center`}>
                      <span className="text-[9px] font-bold text-[#111118]/70">
                        {isCompleted ? '✓' : '100%'}
                      </span>
                    </div>
                    <div className="space-y-1.5 text-[10px] text-[#111118]/60 flex-1">
                      <div className="flex items-center justify-between">
                        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-[#111118]" /> Dev backend / frontend</span>
                        <span className={`font-bold ${isCompleted ? 'text-[#c9922e]' : ''}`}>{isCompleted ? '✓ Terminé' : '50%'}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-[#D95C38]" /> Design & UI</span>
                        <span className={`font-bold ${isCompleted ? 'text-[#c9922e]' : ''}`}>{isCompleted ? '✓ Terminé' : '30%'}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-[#E7B84B]" /> Tests & recette</span>
                        <span className={`font-bold ${isCompleted ? 'text-[#c9922e]' : ''}`}>{isCompleted ? '✓ Terminé' : '20%'}</span>
                      </div>
                    </div>
                  </div>
                );
        ...
        {/* Colonne santé projet, meet, activités (1/3) */}
        <div className="space-y-5">
          ...
          <div className="rounded-[24px] border border-[#111118]/8 bg-white p-6">
            ...
            <div className="mb-4 flex items-center justify-between">
              <span className="text-[11px] text-[#111118]/50">Budget prévu</span>
            ...
            <div>
              <div className="flex items-center justify-between mb-1 text-[11px]">
                ...
                <span className="font-semibold text-[#111118]/80">{timelinePercent}%</span>
              </div>
          ...
          <div className="rounded-[24px] bg-[#111118] p-6 text-white">
            ...
            <h4 className="mt-3 font-heading text-sm font-bold text-white">Session visioconférence</h4>
            <p className="mt-1 text-[11px] text-white/50 leading-relaxed">
              Organisez un appel direct et synchronisé en temps réel entre l'annonceur et le freelance.
            </p>
          ...
          <div className="rounded-[24px] border border-[#111118]/8 bg-white p-6">
            <div className="flex items-center justify-between border-b border-[#111118]/6 pb-2.5 mb-3">
              ...
              <span className="flex items-center gap-1 text-[9px] text-[#D95C38] font-bold">
                <span className="h-1.5 w-1.5 rounded-full bg-[#D95C38] animate-ping" /> Temps réel
              </span>
            </div>
            ...
            {meetings.length === 0 ? (
              ...
              <div className="space-y-3">
                {meetings.map((m) => (
                  <div key={m.id} className="flex items-start gap-2.5 text-[11px] border-b border-[#111118]/6 pb-2.5 last:border-0 last:pb-0">
                    <div className="mt-0.5 h-2 w-2 rounded-full bg-[#D95C38] shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-[#111118] leading-tight">{m.title}</p>
          ...
          >
            ...
            <div className="mb-4">
              ...
              <p className="text-[11px] text-[#111118]/50">
                Le salon sera généré automatiquement et visible pour l'annonceur et le freelance.
              </p>
            </div>

            <form onSubmit={handleAddMeet} className="space-y-4 text-[11px]">
              <div>
                <label className="mb-1 block font-semibold text-[#111118]/70">Titre de la visioconférence</label>
                <input
                  type="text"
                  required
                  value={newMeetTitle}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1 block font-semibold text-[#111118]/70">Date</label>
                  <input
                    type="date"
                    required
                    value={newMeetDate}
                ...
                <div>
                  <label className="mb-1 block font-semibold text-[#111118]/70">Heure</label>
                  <input
                    type="time"
                    required
                    value={newMeetTime}
            ...
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] font-semibold text-[#111118]/60">
                  {activeCall.title} • Room : <span className="font-mono">{activeCall.room_name}</span>
                </p>
              </div>
            </div>
            ...
            <div className="mb-5 space-y-3 rounded-[24px] bg-[#111118] p-5 text-white">
              <div className="flex items-center justify-between">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1 text-[10px] font-bold text-[#E7B84B] border border-white/10">
                  <span className="h-2 w-2 rounded-full bg-[#E7B84B] animate-ping" /> Connexion authentifiée
                </span>
                <span className="text-[10px] text-white/40">Protocole WSS actif</span>
              </div>

              <div className="rounded-xl bg-black/30 p-3 text-[10.5px] font-mono space-y-1 text-white/70 border border-white/10">
                <p><span className="text-white/40">URL serveur :</span> {activeCall.url}</p>
                <p className="truncate"><span className="text-white/40">Jeton JWT :</span> {activeCall.token.substring(0, 35)}...</p>
              </div>
